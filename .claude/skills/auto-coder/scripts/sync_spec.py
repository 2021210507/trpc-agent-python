...
EXPECTED_CHAPTERS = list(NUMBER_SLUG_MAP)
...
def detect_chapters(content: str) -> List[Chapter]:
    ...
    if numbers != EXPECTED_CHAPTERS:
        raise ValueError(
            f"Expected top-level chapters {EXPECTED_CHAPTERS}, found {numbers}"
        )
    chapters = []
...
def sync(force: bool = False):
    ...
    # Write generated chapters atomically.
    for ch in chapters:
        target = specs_dir / ch.filename
