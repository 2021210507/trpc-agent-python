...
其中已包含本示例使用的 tRPC-Agent SDK、SQLAlchemy、`jsonschema`、Docker SDK、pytest 和
flake8；本项目规则脚本本身优先使用标准库。Container 的额外前置是 Docker Desktop，不是另一个
Python requirements 文件，也不得在评审任务执行期间在线安装依赖。
...
| `--sandbox cube` | 当前版本无可验证受控网络证明 | 不应尝试用环境变量绕过 Filter deny |
...
| Cube 被拒绝 | 当前版本的预期安全行为；没有机器证明的网络隔离时不允许执行。 |
